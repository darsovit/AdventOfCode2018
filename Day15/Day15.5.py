...
class unit:
    def __init__(self, goblinOrElf, pos, attackDmg):
        self.state = {}
        self.state['race'] = goblinOrElf
        self.state['hp'] = 200
        self.state['attack'] = attackDmg
        self.state['pos'] = pos
...
class bandits:
    def __init__(self, lines, elfDmg=3, goblinDmg=3):
        self.state = {}
        self.state['board'] = []
        self.state['units'] = {}
        self.state['units']['G'] = {}
        ...
        self.parseLines(lines, goblinDmg, elfDmg)
        
    def parseLines(self, lines, goblinDmg, elfDmg):
        squadNo = 1
        for y in range(len(lines)):
            self.state['board'] += [ lines[y] ]
            for x in range(len(lines[y])):
                if 'G' == lines[y][x]:
                    self.state['units']['G'][squadNo] = unit('G',(y,x), goblinDmg)
                    self.state['unitPos'][(y,x)] = ('G',squadNo)
                    squadNo += 1
                elif 'E' == lines[y][x]:
                    self.state['units']['E'][squadNo] = unit('E',(y,x), elfDmg)
...
game = bandits( readInput(), elfDmg=23 )
playGame( game )
